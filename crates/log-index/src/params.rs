...
impl FilterMapParams {
    ...
    pub fn column_index(&self, lv_index: u64, log_value: &B256) -> u32 {
        ...
        let part_lo = (hash >> (32 - hash_bits)) as u32;
        let collision = (part_hi ^ part_lo) & ((1u32 << hash_bits) - 1);
    ...
    pub fn potential_matches(
        ...
        for (i, row) in rows.iter().enumerate() {
            let max_len = self.max_row_length(i as u32) as usize;
            let mut row_len = row.len();
            if row_len > max_len {
                // any additional entries are generated by another log value on a higher layer
                row_len = max_len;
            }
